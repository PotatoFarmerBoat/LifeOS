...
- **Write every claim so it reads out loud.** A claim is spoken back to the principal in status lines, in the climb strip, and in voice notifications, and he does not have the ISA open while he hears it. So the claim's own words must carry the meaning with no lookup: plain English, the subject named in full the first time, no bare ids, no internal slugs, no unexplained project nouns. The id is a filing label, never the way a claim is referred to in prose ({{PRINCIPAL_NAME}}, 2026-08-23: "C9 or any other titles / nouns that aren't english words are meaningless to me... the claims or requirements are different between projects. so me trying to remember them is never going to happen"). Test: read the claim to someone who has never seen the project. If they cannot say what would make it false, rewrite it.
