...
export function writeWrapper(spec: PlistSpec, bunDir: string): string {
  ...
  if (spec.keepAlive) {
    ...
    // The guard now lives in the .vbs shim (see writeVbsShim), which queries WMI
    // in-process. Spawning PowerShell from here cost ~800ms and a second visible
    // console on every 5-minute tick, for every keepAlive service. Kept as a
    // comment in the generated file so the wrapper is not silently missing the
    // protection a reader expects to find.
    void exe; void token;
    lines.push(`REM already-running guard runs in ${spec.label}.vbs before this file is invoked`);
  }
 ...
 */
function writeVbsShim(label: string, wrapper: string, spec: PlistSpec): string {
  ...
  // The already-running guard runs HERE, in VBScript, instead of as a PowerShell
  // child of the .cmd. A process trace of the 5-minute beat showed the real chain:
  //
  //   wscript (hidden, correct)
  //     └─ cmd.exe        → allocates a conhost
  //          └─ powershell → allocates another, and takes ~800ms
  //
  // Hiding only the wscript link left the other two visible for 2-3 seconds.
  // VBScript talks to WMI directly, so the guard costs one in-process query and
  // spawns nothing at all.
  let guard = "";
  if (spec.keepAlive) {
    const scriptArg = spec.argv.slice(1).find((a) => /\.(ts|js|sh)$/.test(a)) ?? spec.argv[spec.argv.length - 1];
    const token = (scriptArg.split(/[\\/]/).pop() ?? scriptArg);
    const exe = (win(spec.argv[0]).split("\\").pop() ?? "bun.exe");
    guard =
      `Set svc = GetObject("winmgmts:\\\\.\\root\\cimv2")\r\n` +
      `Set procs = svc.ExecQuery("SELECT CommandLine FROM Win32_Process WHERE Name='${q(exe)}'")\r\n` +
      `For Each p In procs\r\n` +
      `  If Not IsNull(p.CommandLine) Then\r\n` +
      `    If InStr(p.CommandLine, "${q(token)}") > 0 Then WScript.Quit 0\r\n` +
      `  End If\r\n` +
      `Next\r\n`;
  }

  const body =
    `' Generated by win-scheduler.ts. Runs the task with no console window at all.\r\n` +
    `' Task Scheduler starts actions in the interactive session, so anything that\r\n` +
    `' allocates a console is visible. wscript never does.\r\n` +
    guard +
    `CreateObject("WScript.Shell").Run "cmd /c """"${q(wrapper)}""""", 0, False\r\n`;
...
export function installTask(spec: PlistSpec, bunDir: string): { code: number; out: string } {
  ...
  const hardened = hardenTaskSettings(spec.label, writeVbsShim(spec.label, wrapper, spec));
  if (hardened.code !== 0) {
