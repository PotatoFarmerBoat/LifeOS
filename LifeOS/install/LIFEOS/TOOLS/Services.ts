...
export function loadedLabels(): Set<string> {
  // Windows has neither launchctl nor systemctl; the equivalent question is
  // which tasks exist under the LifeOS Task Scheduler folder.
...
if (cmd === "status" || cmd === "list") {
  ...
  const targets = REGISTRY.filter(pick).filter((s) => (all || onlyArg ? true : !s.optIn));
  const bun = bunPath();
