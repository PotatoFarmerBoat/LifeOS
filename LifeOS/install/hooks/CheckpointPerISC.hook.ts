...
async function main() {
  ...
  // slugDir derives from the '/'-normalized filePath above, but join() emits '\'
  // on win32 — normalize the prefix too or this test never matches on Windows
  // and the sidecar lands inside the skill tree (windows-port W8, #1119 class).
  const skillsPrefix = join(homedir(), '.claude', 'skills').replaceAll('\\', '/') + '/';
