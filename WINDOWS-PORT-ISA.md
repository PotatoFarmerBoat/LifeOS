...
- [ ] **W1 — detectTool truth.** With bun/git/claude on PATH, `DetectEnv.ts` reports each installed with version+path. Probe: run on Windows, assert JSON fields. Root cause pinned: `Tools/InstallEngine.ts:108` `tryExec("command -v ${name}")` — POSIX builtin, absent from the Windows spawn shell. Fix direction: `Bun.which(name)`. Evidence 2026-08-09: bun reported `installed:false` while executing the probe itself.
- [ ] **W2 — LinkUser junction fallback.** On symlink EPERM (unelevated NTFS), `LinkUser.ts --apply` falls back to a directory junction and exits `ok:true`, contract passed. Probe: run unelevated, assert exit + `Get-Item` reparse point + file count through link. Evidence: EPERM reproduced 2026-08-09; manual `New-Item -ItemType Junction` satisfied the contract (27 items resolve). Upstream precedent: #1730 fixed `setupUserSeparation()` the same way — this is the sibling the class-sweep missed.
- [ ] **W3 — Doctor tells the truth on Windows.** Hook-interpreter check drops `chmod +x` semantics on win32 (probe executability by interpreter resolution, not mode bits); PATH checks use the W1 fix. Probe: Doctor on the sandbox reports hook-interpreter ✅ with bun on PATH. Evidence: 31 false "not executable (chmod +x)" + false "bun: not on PATH" 2026-08-09 — while live hooks fired.
...
- [ ] **W8 — Path-form hygiene sweep.** Enumerate every hook/tool doing string comparison on paths (`file_path` includes/startsWith, `$HOME` concatenation) and normalise per #1119/#1747 pattern. Probe: the enumeration grep IS the plan step (Algorithm rule 9); per-offender unit fixture with `C:\`-form vs `/c/`-form inputs. State: unenumerated.
